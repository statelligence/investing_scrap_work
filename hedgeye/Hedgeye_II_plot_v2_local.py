...
#%%
path1 = 'C:/Users/johnd/OneDrive/Desktop/21_Statelligence/Hedgeye/'
from os import listdir
from os.path import isfile, join
onlyfiles = [f for f in listdir(path1) if isfile(join(path1, f))]
#print(onlyfiles)
...
for i in pd.read_excel(loc, sheet_name = None):
    ...
    k.append(dftemp)
#%%
df_final = pd.concat(k, sort = False)
df_final['SIGNAL'].replace('BULLISH ','BULLISH', inplace = True)
#%%
scol = 'SIGNAL'
sigcond = [df_final[scol] == 'BEARISH', df_final[scol] == 'BULLISH']
sigres = [-1,1]
df_final['BISIG'] = np.select(sigcond, sigres, default = 0)
df_final['DATE2'] = pd.to_datetime(df_final['DATE'], format = '%Y-%m-%d')
#df_test = df_final[df_final.TICKER == 'GOOS'].copy()
#%%
