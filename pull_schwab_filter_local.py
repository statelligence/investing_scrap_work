...
#%%
basepath = "C:/Users/JDSeelig/Desktop/21_Statelligence/Schwab_Filter/"
date = '2021-01-10'
path = basepath +date
#%%
...
for i in onlyfiles:
    ...
    dflist.append(df)
df_final = pd.concat(dflist, sort=True)
df_final.reset_index(inplace = True)
df_final.drop('index', axis = 1, inplace = True)
df_final.fillna(0, inplace = True)
#%%
df_final.drop_duplicates(subset = 'Symbol', inplace = True)
df_final.set_index('Symbol', inplace = True)
#%%
df_final.to_csv(basepath + 'combo_'+date+'.csv')
#%%
...
def fidelread(specPath, fileName):
    ...
    return dfmod2
#%%
dftry = pd.read_csv(path+'/'+ onlyfiles[3])